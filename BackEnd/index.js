...
app.get('/:Linha', async (req, res) => {
  ...
  try {
    async function horarioOnibus() {
      ...
      // Navega até a URL
      await page.goto(`https://www.urbs.curitiba.pr.gov.br/horario-de-onibus/${Linha}`);
      ...
      const horariosDiaUtil = await page.evaluate(() => {
        ...
        return Array.from(elementos).map(elemento => {
          const texto = elemento.textContent.trim().replace(/\s+/g, ' ').replace(/\n/g, ' ');
          ...
          const horariosMatch = texto.match(/DIA ÚTIL(.*?)(Os horários sublinhados|$)/); // Captura os horários
          const ponto = pontoMatch ? pontoMatch[1].trim() : null;
          const horarios = horariosMatch ? horariosMatch[1].trim().split(' ') : [];
      ...
      const horarioSabado=await page.evaluate(()=>{
        ...
        return Array.from(elementos).map(elemento => {
          const texto = elemento.textContent.trim().replace(/\s+/g, ' ').replace(/\n/g, ' ');
          const pontoMatch = texto.match(/^Ponto: (.*?) Válido/); // Captura o nome do ponto
          const horariosMatch = texto.match(/SÁBADO(.*?)(Os horários sublinhados|$)/); // Captura os horários
          const ponto = pontoMatch ? pontoMatch[1].trim() : null;
          const horarios = horariosMatch ? horariosMatch[1].trim().split(' ') : [];
      ...
      return { nomeLinha, horariosDiaUtil,validadeHorario,horarioSabado,horarioDomingo }; // Retorna o nome da linha, os horários e o ponto
    }
