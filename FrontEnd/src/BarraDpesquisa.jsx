...
function BarraPesquisa({setLinha}) {
  ...
  useEffect(() => {
    ...
    if (searchTerm === '') {
      ...
      const filtered = onibusData.filter((item) =>
  ...
  const handleBlur = () => {
    setTimeout(() => {  // Usando `setTimeout` para garantir que o clique na lista seja processado antes de o input perder o foco
      setHasFocus(false);
  ...
  return (
    <div>
      ...
      {hasFocus && isTyping && filteredData.length > 0 && (
        <ul style={{ paddingLeft: 0,border:"1px solid"}}>
      ...
      {hasFocus && isTyping && filteredData.length === 0 && (
        <p style={{ color: 'red' }}>Nenhum ponto de ônibus encontrado.</p>
      )}
    </div>
  );
}
